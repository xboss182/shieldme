...
// on MailBaby's outbound spam filter. Strip Reply-To for these to avoid the +4 penalty.
...
async function processForwardingJob(job: Job<EmailForwardingJob>) {
  ...
  const rawResult = payload.rawMessage && pgpMode === 'none'
  ...
  // Merge raw-message tracking metadata into the tracking headers if applicable
