...
export default function AliasList({ token, refreshTrigger }: Props) {
  ...
  if (loading) return (
    <div className="px-4 py-6 text-center text-sm text-gray-400">Loading aliases…</div>
  );

  if (error) return (
    <div className="px-4 py-3 text-xs text-red-600 bg-red-50 rounded mx-4">{error}</div>
  );

  if (aliases.length === 0) return (
    <div className="px-4 py-6 text-center text-sm text-gray-400">No aliases yet. Create one above.</div>
  );
        ...
        return (
          <div key={alias.id} className="flex items-center gap-2 px-4 py-2.5 hover:bg-gray-50">
            <div className="flex-1 min-w-0">
              ...
              {alias.label && (
                <span className="text-xs text-gray-400">{alias.label}</span>
              )}
            </div>
            <button
